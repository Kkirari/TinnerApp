...
export class LoginComponent {
  ...
  async onSubmit() {
    ...
    if (this.mode === 'login') {
      ...
      //register
      this.errorFormServer = await this.accountService.register(
        this.form.value
      );
    }

    if (this.errorFormServer === '') this.router.navigate(['/']);
  }
  ...
  updateErrorMessage(ctrlName: string) {
    ...
    switch (ctrlName) {
      ...
      case 'password':
        if (control.hasError('required'))
          ...
          this.errorMessages.password.set(
            'must contain minimum of 1 lower-case letter'
          );
        else if (control.hasError('invalidUpperCase'))
          this.errorMessages.password.set(
            'must contain minimum of 1 capital letter'
          );
        else if (control.hasError('invalidNumeric'))
          this.errorMessages.password.set(
            'must contain minimum of 1 numeric character'
          );
      ...
      case 'confirm_password':
        ...
        else this.errorMessages.confirm_password.set('');
        break;
      case 'display_name':
        if (control.hasError('required'))
